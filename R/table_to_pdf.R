...
mat_table_to_pdf <- function(x, filename = "input.tex", clean=TRUE, quiet=TRUE,
  ...
  if(str_detect(filename, "\\.pdf$|\\.png$")) warning("filneame should be raw")
  if(!str_detect(filename, "\\.tex"))   filename <- paste(filename, ".tex", sep="")
  ...
  try(texi2pdf(basename(filename), quiet=quiet, clean=clean))
  setwd(old_wd)

  if(clean) {
    filename_2  <- str_remove(filename, "\\..+$")
    all_files <- list.files(path = dirname(filename),
                            pattern=basename(filename_2),
                            full.names = TRUE)
    # all_files <- list.files(pattern=filename_2)
    all_files_rem <- all_files[!str_detect(all_files, "\\.pdf")]
